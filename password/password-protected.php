...
$date = date("Y-m-d");
$sql = "SELECT * FROM poll where password is not null and PollDate <= '$date';";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo '
                    <div class="vote">
                        ...
                        <input type="hidden" value="'.$row["password"].'" name="acutal-password">
                        <h1>' . $row["title"] . '</h1>
                ...
                echo '<h1>NO PASSWORD PROTECTED POLLS</h1>';
            }
            ?>
    </div>
        </body>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if ($number > 0){
        echo"<script>alert('YOU HAVE ALREADY VOTED ONCE')</>";
    }else{
    $password = $_POST['password'];
    $actual = $_POST['acutal-password'];
    if (password_verify($password, $actual)){
